...
def main(host: str, port: int, trans: str, num: int, tool_concurrency: bool = False) -> None:
    async def do_single_test(duration: float | None, transport):
    ...
    async def do_multi_test():
        global in_progress_leave
        global post_queue_enter, post_queue_leave
        try:
            async with Client(transport) as client:
                # task
                async def do_test(duration: float | None):
                    await client.call_tool("sleep_lightly", {"duration": duration})
                ...
                async with anyio.create_task_group() as tg:
                    for _ in range(num):
                        duration = None
                        tg.start_soon(do_test, duration)
        except Exception as e:
            print(str(e), end="; ")
        finally:
            in_progress_leave += 1
        ...
        if tool_concurrency:
            ...
            async with anyio.create_task_group() as tg:
                for _ in range(num):
                    ...
                    #duration = np.random.exponential(scale=1.0)
                    duration = None
                    tg.start_soon(do_single_test, duration, transport)
        print("done.")
